...
DB 운영 안정성을 높이기 위해 PostgreSQL은 관리형 서비스(RDS)를 기본으로 권장한다.
RDS for PostgreSQL은 백업, 복구, Multi-AZ 같은 운영 기능을 제공해 MVP~파일럿 단계에서 운영 리스크를 줄이기 좋다. ([AWS Documentation][10])

### 권장 운영 구조

* App 컨테이너(웹/API/워커) + Docker Compose 기반 운영
* RDS PostgreSQL
* S3
* RabbitMQ
* Prometheus / Grafana
...
Prometheus는 시계열 모니터링에 적합하고, Grafana는 대시보드 시각화에 적합하다. ([OpenTelemetry][6])
...
파일럿 전 단계에서는 과합니다.
Docker Compose + 관리형 RDS 중심으로도 충분합니다. ([AWS Documentation][10])
...
* **Queue:** Celery + RabbitMQ
* **Storage:** S3
* **Infra:** Docker Compose + Amazon RDS for PostgreSQL
* **Observability:** Prometheus + Grafana
